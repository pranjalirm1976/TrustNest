...
import { CreditCard, AlertCircle, UtensilsCrossed, ExternalLink, ArrowUpRight } from 'lucide-react'
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col gap-0.5">
                <span className="text-sm font-bold text-slate-900 leading-tight group-hover:text-brand-primary transition-colors">
                  {act.label}
                </span>
                <span className="text-[10px] text-slate-400 font-medium leading-tight">{act.desc}</span>
              </div>
            </div>
            <ArrowUpRight className="w-4 h-4 text-slate-300 group-hover:text-brand-primary opacity-0 group-hover:opacity-100 transition-all shrink-0" />
          </Link>
        ))}
      </div>
    </div>
